...
interface ProjectState {
  ...
  closeProject: (id: string) => void; renameProject: (id: string, title: string) => Promise<void>; deleteProject: (id: string) => Promise<void>; selectProject: (id: string) => void;
...
export const useProjectStore = create<ProjectState>()(persist((set, get) => ({
  ...
  loadProjects: async () => { set({ loadingProjects:true,projectError:null });try{const projects=await projectsApi.list();set((state)=>{const ids=new Set(projects.map((p)=>p.id));const openProjectIds=state.openProjectIds.filter((id)=>ids.has(id));return{projects,openProjectIds,activeProjectId:state.activeProjectId&&ids.has(state.activeProjectId)?state.activeProjectId:openProjectIds[openProjectIds.length-1]??null,loadingProjects:false}});const id=get().activeProjectId;if(id){void get().loadProjectContent(id);void useThreadStore.getState().loadThreads(id)}}catch(error){set({loadingProjects:false,projectError:error instanceof Error?error.message:'项目加载失败，请重试'})}},
  ...
  closeProject: (id) => { set((state)=>{const openProjectIds=state.openProjectIds.filter((value)=>value!==id);return{openProjectIds,activeProjectId:state.activeProjectId===id?openProjectIds[openProjectIds.length-1]??null:state.activeProjectId}}) },
