...
export const useProjectStore = create<ProjectState>()(persist((set, get) => ({
  ...
  setProjectTheme: async (projectId,themeId) => { const project=await projectsApi.setTheme(projectId,themeId);set((state)=>{const current=state.contentByProjectId[projectId];return{projects:state.projects.map((value)=>value.id===projectId?project:value),contentByProjectId:current?{...state.contentByProjectId,[projectId]:{...current,design:{...current.design,theme:project.theme,revision:project.design_revision??current.design.revision,updated_at:project.updated_at}}}:state.contentByProjectId}});return project },
