...
import { projectSourceDraftCount, sourceDirty, sourceKey, useSourceEditorStore } from '../../stores/sourceEditorStore';
...
export function SourceWorkspace({ projectId, slideId, kind, blocked }: { projectId: string; slideId: string; kind: SourceKind; blocked: boolean }) {
  ...
  const activeDrafts = [...new Map<string, { id: string; slideId: string; kind: SourceKind }>([
    ...
    const position = (id: string) => { const found = slides.findIndex((slide) => slide.id === id); return found < 0 ? Number.MAX_SAFE_INTEGER : found; };
    return position(a.slideId) - position(b.slideId) || (a.kind === b.kind ? 0 : a.kind === 'spec' ? -1 : 1);
  });
  ...
  const filename = kind === 'spec' ? 'spec.json' : 'index.html';
  ...
  return <div className="flex min-h-0 min-w-0 flex-1 flex-col bg-surface">
    <div className="flex min-h-11 flex-wrap items-center gap-2 border-b border-border bg-panel px-3 py-1.5 text-xs">
      <span className="min-w-0 truncate font-mono font-semibold text-text-900" title={file ? `slides/${slideId}/${filename}` : filename}>{filename}</span>
      <span className="text-text-400">{kind === 'spec' ? 'JSON' : 'HTML'}</span>
    ...
    {showDrafts && <div className="border-b border-border bg-panel px-3 py-2 text-xs">
      {activeDrafts.map((draft) => <button key={draft.id} type="button" className="mr-3 rounded px-1.5 py-1 text-text-700 hover:bg-accent-soft" onClick={() => {
        setCurrentSlideId(draft.slideId); setGlobalView(draft.kind === 'spec' ? 'outline' : 'html'); setShowDrafts(false);
      }}>{slides.find((slide) => slide.id === draft.slideId)?.title || draft.slideId} · {draft.kind === 'spec' ? 'spec.json' : 'index.html'}</button>)}
    ...
    {showBackups && <div className="border-b border-border bg-panel px-3 py-2 text-xs">
      ...
      {backups.map((draft) => <div key={draft.id} className="flex items-center gap-2 py-1">
        <button type="button" className="truncate text-left hover:underline" onClick={() => setSelectedBackup(draft)}>{slides.find((slide) => slide.id === draft.slideId)?.title || draft.slideId} · {draft.kind === 'spec' ? 'spec.json' : 'index.html'} · {new Date(draft.updatedAt).toLocaleString()}</button>
