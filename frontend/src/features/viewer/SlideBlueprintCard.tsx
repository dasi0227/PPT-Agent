  ...
  return (
    <article className={`h-full w-full rounded-xl border border-border bg-surface ${compact ? 'p-3' : 'p-7'} shadow-sm`}>
      <div className="flex items-center justify-between gap-2">
        <span className="rounded bg-mode-outline/10 px-2 py-1 text-[10px] font-semibold uppercase tracking-wide text-mode-outline">
          {blueprint.role}
        </span>
      ...
      <div className={`${compact ? 'mt-2 text-[10px]' : 'mt-4 text-base'} flex gap-2 rounded-lg bg-mode-outline/5 p-3 text-text-900`}>
        <MessageSquareText className="h-4 w-4 shrink-0 text-mode-outline" />
      ...
      {!compact && (
        <>
          ...
          <div className="mt-5 flex gap-2 border-t border-border pt-4 text-sm text-text-600">
            <Eye className="h-4 w-4 shrink-0 text-mode-page" />
