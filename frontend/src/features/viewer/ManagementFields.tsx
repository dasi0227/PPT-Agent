...
export function ManagementSection({ title, count, action, children, className = '' }: {
  ...
  return <section className={`management-section ${className}`} aria-label={title}>
    <div className="management-section-heading"><h2>{title}{count !== undefined && <span className="management-count">{count}</span>}</h2>{action}</div>
    {children}
  </section>;
}

function ReadableValue({ id, value, label, disabled, onEdit }: { id: string; value: string; label: string; disabled: boolean; onEdit: () => void }) {
  return <div className="management-value"><span className={!value ? 'text-text-600' : undefined}>{value || '待填写'}</span>
...
export function TextProperty<T>({ editor, id, label, value, displayValue, update, minLength = 0, maxLength, multiline = false }: {
  ...
  return <div className="management-field"><div className="management-label">{label}</div><div className="min-w-0">
    {editor.draft?.id === id ? <InlineTextEditor editor={editor} /> : <ReadableValue id={buttonId} value={displayValue ?? value} label={label} disabled={editor.disabled}
      onEdit={() => editor.start({ id, label, value, update, minLength, maxLength, multiline })} />}
  </div></div>;
}

export function ListProperty<T, Item>({ editor, id, label, items, maxLength, getText, withText, createItem, update, leading, draftLeading }: {
  ...
  leading?: (item: Item, index: number) => ReactNode; draftLeading?: ReactNode;
}) {
  const listId = useId();
  const adding = editor.draft?.id === `${id}:new`;
  return <ManagementSection title={label} count={items.length} action={
    <button type="button" className="management-add" id={`${listId}-add`} aria-label={`新增${label}`} disabled={editor.disabled || items.length >= 32}
    ...
    {items.length || adding ? <ul className="management-list" role="list">
      {items.map((item, index) => {
        ...
        return <li key={itemId} className={`management-item ${leading ? 'management-element' : ''}`}>
          {leading ? leading(item, index) : <span className="management-bullet" aria-hidden="true" />}
          <div className="min-w-0">{editor.draft?.id === itemId ? <InlineTextEditor editor={editor} />
            : <ReadableValue id={`${listId}-edit-${index}`} value={getText(item)} label={itemLabel} disabled={editor.disabled}
          ...
          <IconButton data-focus-fallback={`${listId}-add`} label={`删除${itemLabel}`} className="management-delete-action" disabled={editor.disabled}
      ...
      {adding && <li className={`management-item ${leading ? 'management-element' : ''}`}>
        {draftLeading ?? <span className="management-bullet" aria-hidden="true" />}<div className="min-w-0"><InlineTextEditor editor={editor} /></div>
      </li>}
