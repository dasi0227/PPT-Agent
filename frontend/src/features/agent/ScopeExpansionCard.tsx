  ...
  return (
    <article className="overflow-hidden rounded-xl border border-border bg-surface">
      ...
      <div className="flex flex-wrap justify-end gap-1.5 border-t border-border bg-panel-muted px-3 py-2" role="group" aria-label="范围扩权操作">
        <button type="button" disabled={submitting !== null} onClick={() => void submit('reject')} className="inline-flex h-8 items-center gap-1 rounded-md ui-danger-solid px-2.5 text-xs font-semibold focus-visible:outline-none disabled:opacity-50">
          <X className="h-3.5 w-3.5" />{submitting === 'reject' ? '提交中' : '拒绝'}
        </button>
        {item.proposedScope.source.kind !== 'all_pages' && (
          <button type="button" disabled={submitting !== null} onClick={() => void submit('adjust')} className="inline-flex h-8 items-center rounded-md border border-border bg-surface px-2.5 text-xs font-semibold ui-warning focus-visible:outline-none disabled:opacity-50">
            {submitting === 'adjust' ? '提交中' : '调整为全部页'}
          </button>
        )}
        <button type="button" disabled={submitting !== null} onClick={() => void submit('approve')} className="inline-flex h-8 items-center gap-1 rounded-md ui-success-solid px-3 text-xs font-semibold focus-visible:outline-none disabled:opacity-50">
