...
export function modelChanged(row: DraftModel, edit?: Editable): boolean {
  return !row.previous_name || !!edit && (['name', 'protocol', 'base_url', 'model', 'key'] as const).some((field) => edit[field] !== row[field]);
}
...
export function validation(row: DraftModel, rows: DraftModel[]): string {
  ...
  if (!row.model.trim()) return '请填写模型标识。';
...
export function settingsPayload(draft: Draft): SettingsEdit {
  return {
    ...
    llm: draft.llm.map((row) => ({
      name: row.name, protocol: row.protocol, base_url: row.base_url, model: row.model,
