...
var errorDefinitions = map[string]ErrorDefinition{
	...
	"SPEC_CONTENT_CONFLICT":          {Code: "SPEC_CONTENT_CONFLICT", Category: ErrorConflict, SafeMessage: "规格要求已被更新，请刷新后重试。", ModelMessage: "Read the current spec content and hash before applying another patch.", HTTPStatus: 409},
	"SPEC_REFERENCE_BROKEN":          {Code: "SPEC_REFERENCE_BROKEN", Category: ErrorAgentRepairable, SafeMessage: "规格要求引用了不存在的内容。", ModelMessage: "Correct the broken slide or asset reference.", HTTPStatus: 422},
	"SPEC_INVALID":                   {Code: "SPEC_INVALID", Category: ErrorAgentRepairable, SafeMessage: "规格要求未通过格式检查。", ModelMessage: "Correct the reported spec validation issue.", HTTPStatus: 422},
	"INVALID_SCOPE":                  {Code: "INVALID_SCOPE", Category: ErrorUserActionRequired, SafeMessage: "任务范围不合法。", ModelMessage: "Correct scope.selection using stable slide or section IDs.", HTTPStatus: 422},
	...
	"MODEL_VISION_REQUIRED":          {Code: "MODEL_VISION_REQUIRED", Category: ErrorUserActionRequired, SafeMessage: "当前模型不支持图片，请更换模型后发送。", ModelMessage: "Choose a vision-capable model before sending image attachments.", HTTPStatus: 422},
	"ATTACHMENT_TYPE_UNSUPPORTED":    {Code: "ATTACHMENT_TYPE_UNSUPPORTED", Category: ErrorUserActionRequired, SafeMessage: "仅支持 PNG、JPG 和 WebP 图片。", ModelMessage: "Use a PNG, JPEG, or WebP static image.", HTTPStatus: 422},
	"ATTACHMENT_TOO_LARGE":           {Code: "ATTACHMENT_TOO_LARGE", Category: ErrorUserActionRequired, SafeMessage: "图片超过 10 MiB，请压缩后重试。", ModelMessage: "Choose an image smaller than 10 MiB.", HTTPStatus: 422},
	"ATTACHMENT_DIMENSIONS_EXCEEDED": {Code: "ATTACHMENT_DIMENSIONS_EXCEEDED", Category: ErrorUserActionRequired, SafeMessage: "图片分辨率过高，请缩小后重试。", ModelMessage: "Choose an image with at most 40 megapixels.", HTTPStatus: 422},
	"ATTACHMENT_INVALID":             {Code: "ATTACHMENT_INVALID", Category: ErrorUserActionRequired, SafeMessage: "无法读取这张图片，请更换文件。", ModelMessage: "Choose a valid static image with matching content and MIME type.", HTTPStatus: 422},
	"ATTACHMENT_NOT_FOUND":           {Code: "ATTACHMENT_NOT_FOUND", Category: ErrorUserActionRequired, SafeMessage: "附件已不可用，请重新选择。", ModelMessage: "Choose an attachment that belongs to the current project.", HTTPStatus: 404},
	"ATTACHMENT_LIMIT_EXCEEDED":      {Code: "ATTACHMENT_LIMIT_EXCEEDED", Category: ErrorUserActionRequired, SafeMessage: "每条消息最多添加 8 张图片。", ModelMessage: "Limit the message to eight image attachments.", HTTPStatus: 422},
	"DOM_SELECTION_INVALID":          {Code: "DOM_SELECTION_INVALID", Category: ErrorUserActionRequired, SafeMessage: "DOM 标记数据无效，请重新选择。", ModelMessage: "Choose DOM elements from the current project preview.", HTTPStatus: 422},
	"DOM_SELECTION_LIMIT":            {Code: "DOM_SELECTION_LIMIT", Category: ErrorUserActionRequired, SafeMessage: "选择内容过多，请缩小范围。", ModelMessage: "Limit the message to eight DOM selections and fifty targets per selection.", HTTPStatus: 422},
	"DOM_SELECTION_TOO_LARGE":        {Code: "DOM_SELECTION_TOO_LARGE", Category: ErrorUserActionRequired, SafeMessage: "选择内容过大，请缩小范围。", ModelMessage: "Reduce the selected DOM snapshot size.", HTTPStatus: 422},
	"DOM_SELECTION_COMMENT_TOO_LONG": {Code: "DOM_SELECTION_COMMENT_TOO_LONG", Category: ErrorUserActionRequired, SafeMessage: "标记注释最多 500 个字符。", ModelMessage: "Shorten the DOM selection comment to 500 characters.", HTTPStatus: 422},
	"REFERENCE_ORDER_INVALID":        {Code: "REFERENCE_ORDER_INVALID", Category: ErrorUserActionRequired, SafeMessage: "消息引用顺序无效，请重试。", ModelMessage: "Reference every image and DOM selection exactly once.", HTTPStatus: 422},
	...
	"SKILL_DISABLED":                 {Code: "SKILL_DISABLED", Category: ErrorUserActionRequired, SafeMessage: "所选技能已禁用，请重新选择。", ModelMessage: "Choose an enabled skill returned by the repository catalog.", HTTPStatus: 409},
	...
	"RESOURCE_INVALID":               {Code: "RESOURCE_INVALID", Category: ErrorAgentRepairable, SafeMessage: "PPT 资源地址不合法。", ModelMessage: `read_resource requires resource: "manifest" | "design" | "outline" | "spec" | "html". Only spec/html require slide_id; global resources forbid it. Use an existing Runtime-issued slide_id, never an ordinal or invented ID.`, HTTPStatus: 422},
	...
	"TARGET_OUT_OF_SCOPE":            {Code: "TARGET_OUT_OF_SCOPE", Category: ErrorAgentRepairable, SafeMessage: "目标超出当前任务范围。", ModelMessage: "Use an authorized slide_id, or call request_privilege with a reason and the required scope additions if it is disclosed. Wait for approval before writing outside the current scope.", HTTPStatus: 409},
	"RESOURCE_NOT_FOUND":             {Code: "RESOURCE_NOT_FOUND", Category: ErrorAgentRepairable, SafeMessage: "未找到所需内容。", ModelMessage: "Use an existing resource or repository ID from the current context; follow the tool-specific creation or refresh instructions. Do not repeat an unchanged read of missing content.", HTTPStatus: 404},
	"OUTLINE_NOT_INITIALIZED":        {Code: "OUTLINE_NOT_INITIALIZED", Category: ErrorAgentRepairable, SafeMessage: "目录结构尚未初始化。", ModelMessage: "There is no saved outline yet. Use init_outline with complete JSON source when it is disclosed and creation is needed; otherwise continue without an outline. Do not retry reading it or invent slide IDs.", HTTPStatus: 404},
	"RESOURCE_CONTENT_INVALID":       {Code: "RESOURCE_CONTENT_INVALID", Category: ErrorUserActionRequired, SafeMessage: "已保存的资源内容损坏，需要修复后再读取。", ModelMessage: "The saved resource exists but cannot be parsed or validated. Report the damaged resource; do not treat it as missing, reinitialize it, or repeatedly read unchanged content.", HTTPStatus: 422},
	"READ_FAILED":                    {Code: "READ_FAILED", Category: ErrorTerminal, SafeMessage: "资源读取失败，请检查存储状态。", ModelMessage: "Resource I/O failed. Inspect storage or permissions; do not treat unreadable content as absent or recreate it.", HTTPStatus: 500},
	"WRITE_FAILED":                   {Code: "WRITE_FAILED", Category: ErrorTerminal, SafeMessage: "资源写入失败，请检查存储状态。", ModelMessage: "Resource I/O failed while writing. Inspect storage and the saved state before another write; do not claim this operation succeeded.", HTTPStatus: 500},
	...
	"PATCH_INVALID":                  {Code: "PATCH_INVALID", Category: ErrorAgentRepairable, SafeMessage: "PPT 修改指令格式不正确。", ModelMessage: "Follow the disclosed edit tool parameters. JSON editors accept business fields; patch_html and arrange_outline accept edits with exact old_text and new_text strings.", HTTPStatus: 422},
	"PATCH_PATH_DENIED":              {Code: "PATCH_PATH_DENIED", Category: ErrorAgentRepairable, SafeMessage: "PPT 修改位置不允许写入。", ModelMessage: "Choose an editable field disclosed by the resource tool schema.", HTTPStatus: 422},
	"EDIT_ANCHOR_NOT_FOUND":          {Code: "EDIT_ANCHOR_NOT_FOUND", Category: ErrorAgentRepairable, SafeMessage: "未找到要替换的内容。", ModelMessage: "Call read_resource and use an exact current anchor.", HTTPStatus: 422},
	"EDIT_ANCHOR_AMBIGUOUS":          {Code: "EDIT_ANCHOR_AMBIGUOUS", Category: ErrorAgentRepairable, SafeMessage: "要替换的内容不唯一。", ModelMessage: "Call read_resource and choose a longer unique anchor.", HTTPStatus: 422},
	...
	"TOOL_ARGUMENT_INVALID":          {Code: "TOOL_ARGUMENT_INVALID", Category: ErrorAgentRepairable, SafeMessage: "工具参数格式不正确，正在修正。", ModelMessage: "Correct the reported field to match the disclosed tool schema, then call the tool again. Pass arrays and objects as JSON values, not quoted JSON strings. Do not repeat unchanged arguments.", HTTPStatus: 422},
	"PLAN_INVALID":                   {Code: "PLAN_INVALID", Category: ErrorAgentRepairable, SafeMessage: "计划更新不符合当前规则，正在修正。", ModelMessage: "Correct the reported plan issue using the current plan and disclosed tool parameters. Use existing step IDs and valid statuses; do not replace an active plan or mark unfinished work complete.", HTTPStatus: 422},
	...
	"SCOPE_EXPANSION_REQUIRED":       {Code: "SCOPE_EXPANSION_REQUIRED", Category: ErrorAgentRepairable, SafeMessage: "需要申请扩大任务范围。", ModelMessage: "Call request_privilege with a reason and the required scope additions when disclosed; wait for approval. Updating plan progress does not grant permission.", HTTPStatus: 422},
	...
	"COMMAND_PARSE_INVALID":          {Code: "COMMAND_PARSE_INVALID", Category: ErrorAgentRepairable, SafeMessage: "命令语法不正确。", ModelMessage: "Correct the reported command syntax and quoting, then submit the corrected command.", HTTPStatus: 422},
	"COMMAND_SYNTAX_DENIED":          {Code: "COMMAND_SYNTAX_DENIED", Category: ErrorAgentRepairable, SafeMessage: "命令使用了不支持的语法。", ModelMessage: "Use only syntax supported by run_command; simplify the command as reported. Submit an approval-bound edit as a single command.", HTTPStatus: 422},
	"COMMAND_NOT_ALLOWED":            {Code: "COMMAND_NOT_ALLOWED", Category: ErrorAgentRepairable, SafeMessage: "当前不支持执行该命令。", ModelMessage: "Choose a supported command from the disclosed run_command schema; do not bypass the command policy.", HTTPStatus: 422},
	"COMMAND_NOT_AVAILABLE":          {Code: "COMMAND_NOT_AVAILABLE", Category: ErrorAgentRepairable, SafeMessage: "该命令当前不可用。", ModelMessage: "Choose an available supported command or use the relevant resource tool; do not repeat the unavailable command.", HTTPStatus: 422},
	"COMMAND_FLAG_DENIED":            {Code: "COMMAND_FLAG_DENIED", Category: ErrorAgentRepairable, SafeMessage: "命令参数不受支持。", ModelMessage: "Remove or correct the reported unsupported flag while keeping the operation within the command policy.", HTTPStatus: 422},
	"COMMAND_PATH_OUTSIDE_PROJECT":   {Code: "COMMAND_PATH_OUTSIDE_PROJECT", Category: ErrorAgentRepairable, SafeMessage: "命令路径超出项目范围。", ModelMessage: "Use a path inside the current project. Do not bypass the boundary through traversal, links or another command.", HTTPStatus: 422},
	"COMMAND_PATH_INVALID":           {Code: "COMMAND_PATH_INVALID", Category: ErrorAgentRepairable, SafeMessage: "命令指定的路径无效。", ModelMessage: "Inspect the allowed project directory and use an existing valid project-relative path.", HTTPStatus: 422},
	"COMMAND_SENSITIVE_READ_DENIED":  {Code: "COMMAND_SENSITIVE_READ_DENIED", Category: ErrorAgentRepairable, SafeMessage: "该命令无权读取敏感内容。", ModelMessage: "Respect the read restriction. Continue using authorized content; do not seek an alternate route to the denied data.", HTTPStatus: 403},
	"COMMAND_PERMISSION_DENIED":      {Code: "COMMAND_PERMISSION_DENIED", Category: ErrorAgentRepairable, SafeMessage: "用户未批准本次命令。", ModelMessage: "The user denied this command. Continue within existing authorization or explain the limitation; do not repeat or bypass the denied operation.", HTTPStatus: 403},
	"COMMAND_EXIT_NONZERO":           {Code: "COMMAND_EXIT_NONZERO", Category: ErrorAgentRepairable, SafeMessage: "命令返回了非零退出状态。", ModelMessage: "Inspect exit_code, stdout and stderr. Interpret a no-match result correctly; otherwise correct the command or its input before retrying.", HTTPStatus: 422},
	"COMMAND_TIMEOUT":                {Code: "COMMAND_TIMEOUT", Category: ErrorAgentRepairable, SafeMessage: "命令执行超时。", ModelMessage: "Reduce the command scope or choose a narrower query before retrying.", HTTPStatus: 422},
	"COMMAND_OUTPUT_LIMIT":           {Code: "COMMAND_OUTPUT_LIMIT", Category: ErrorAgentRepairable, SafeMessage: "命令输出超过上限。", ModelMessage: "Use narrower paths, filters or bounded output. Do not treat truncated output as complete.", HTTPStatus: 422},
	"COMMAND_EXEC_FAILED":            {Code: "COMMAND_EXEC_FAILED", Category: ErrorAgentRepairable, SafeMessage: "命令未能执行成功。", ModelMessage: "Inspect the reported execution failure and choose a corrected supported command. If the environment is unavailable, report the limitation.", HTTPStatus: 422},
	"COMMAND_INVARIANT_VIOLATION":    {Code: "COMMAND_INVARIANT_VIOLATION", Category: ErrorTerminal, SafeMessage: "命令执行状态异常，无法继续该操作。", ModelMessage: "Stop the command and inspect the Runtime preflight and approval state. Do not bypass the missing execution guard.", HTTPStatus: 500},
	...
	"MODEL_TOOL_CALL_INVALID":        {Code: "MODEL_TOOL_CALL_INVALID", Category: ErrorTerminal, SafeMessage: "模型返回的工具调用无法解析，本次任务已停止。", ModelMessage: "The response could not be decoded into valid tool calls. No tools from this response were executed. Inspect the malformed call identity or JSON; do not retry this as a provider outage.", HTTPStatus: 502},
	"IMAGE_REFERENCE_UNAVAILABLE":    {Code: "IMAGE_REFERENCE_UNAVAILABLE", Category: ErrorTerminal, SafeMessage: "图片引用已失效或无法读取，请重新渲染页面或重新添加图片后再试。", ModelMessage: "A local image reference could not be read before contacting the provider. Refresh the render or attachment reference.", HTTPStatus: 422},
	...
	"FINISH_NOT_ALLOWED":             {Code: "FINISH_NOT_ALLOWED", Category: ErrorAgentRepairable, SafeMessage: "当前阶段尚不能结束任务。", ModelMessage: "Use the control tools disclosed for the current phase; do not call finish before execution is allowed to complete.", HTTPStatus: 422},
	"RUN_LANGUAGE_UNSATISFIED":       {Code: "RUN_LANGUAGE_UNSATISFIED", Category: ErrorAgentRepairable, SafeMessage: "文稿语言尚未符合任务要求。", ModelMessage: "Use edit_manifest to set the requested language, and ensure the actual slide content follows that language before finish.", HTTPStatus: 422},
	"RUN_RANGE_UNSATISFIED":          {Code: "RUN_RANGE_UNSATISFIED", Category: ErrorAgentRepairable, SafeMessage: "实际页面数量尚未符合任务要求。", ModelMessage: "Use the disclosed init_outline or arrange_outline to bring the actual slide count into the requested range, then complete the affected pages. Changing Manifest.pages alone does not change the actual count.", HTTPStatus: 422},
	"CONTEXT_SOURCE_INVALID":         {Code: "CONTEXT_SOURCE_INVALID", Category: ErrorUserActionRequired, SafeMessage: "当前资源无法用于完成检查。", ModelMessage: "Inspect the reported source resource and its read error. Do not replace damaged saved content with an empty resource or claim completion; report storage problems if the resource cannot be safely read.", HTTPStatus: 422},
	...
	"WORK_NOT_COMPLETE":              {Code: "WORK_NOT_COMPLETE", Category: ErrorAgentRepairable, SafeMessage: "仍有页面任务未完成。", ModelMessage: "Inspect unfinished work_ledger items and last_error; complete the actual page operations and synchronize related plan statuses. There is no update_work tool and no requirement to edit other authorized pages.", HTTPStatus: 422},
