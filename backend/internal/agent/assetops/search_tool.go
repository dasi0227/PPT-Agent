...
// SearchAssetsTool 按 kind + 关键词检索个人仓库资产索引（search_assets）。
// 该工具可在 repo/current/page/overview 下使用；写入仍由各 scope 的写工具负责。
...
func (t *SearchAssetsTool) Scopes() []model.Scope {
	return []model.Scope{model.ScopeRepo, model.ScopeCurrent, model.ScopePage, model.ScopeOverview}
}
