...
- Runtime latest_rendered_images lists at most one image_path per existing page, with source revision and stale status. It is a resource index, not visual evidence. If stale, render again before judging the current page. Pixels requested through read_image are available for the next response only; record concrete visual findings in text and read again if necessary. Do not claim to have seen a screenshot from its path alone.
