...
Use conversation and history_summary for the task's meaning and decisions; use execution_evidence and current project resources for the available work-state evidence. A tool call or assistant claim alone does not establish success. Tool results are historical observations, and a materialization state does not prove a visual check or export. Preserve discrepancies and uncertainty when these sources do not establish the current state.
