...
无登录注册、单用户单机。需保存项目（含演示文稿结构）、Slide、版本历史与个人资产仓库。slide 是大文本 HTML，需可直接渲染、git 友好；元数据需可查询、事务安全。

## 决策

**混合持久化**：元数据（项目/Slide 索引/版本记录/Run/资产索引）入 **SQLite**；slide html/css/js、公共样式层、slide-json、资产资源等大文本入**文件系统**（work_dir）。两者经路径字段关联。
