...
| SQLite 驱动 | **`modernc.org/sqlite`**（纯 Go，无 cgo） | 纯 JDBC 驱动 |
...
- **驱动**：GORM 默认 `gorm.io/driver/sqlite` 基于 cgo（mattn）。改用纯 Go：以 `modernc.org/sqlite`（DriverName=`sqlite`）配合 GORM SQLite dialector 接入，免 cgo、便于交叉编译与部署。
...
| 驱动 | modernc | mattn/go-sqlite3 | mattn 依赖 cgo，交叉编译/部署麻烦 |
...
- go.mod 引入：`gorm.io/gorm`、`modernc.org/sqlite`、`github.com/spf13/viper`、`go.uber.org/zap`、`github.com/google/wire`。
