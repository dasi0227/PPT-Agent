...
> 2026-09-22 更新：页面运行截图的引用、按需读取及生命周期以 [页面渲染图片按需读取设计](2026-09-22-render-image-on-demand-design.md) 为准；本文的用户上传附件行为不变。

- 状态：已实现
- 日期：2026-09-09
- 交互更新：2026-09-22，附件图标替换为缩略图，点击查看原图。
...
3. 上传成功的附件在输入框内显示图片缩略图、文件名和大小，点击缩略图可弹窗查看原图，发送前可解除当前消息引用。
...
- 每张附件在左侧 38 × 38 区域等比完整显示缩略图，右侧保留截断文件名与文件大小；缩略图加载失败时隐藏图片区域，保留文件信息与移除操作。
- 点击缩略图使用公共 Dialog 弹窗按窗口尺寸等比展示原图，打开时才请求原图；弹窗显示文件名，支持关闭按钮、点击遮罩和 Escape 关闭。移除操作不触发预览。
- 图片预览仅请求附件内容接口，不修改 Agent 上下文或增加 LLM 图片输入。
...
待上传 File 仅存在于短生命周期 upload job，不进入可见附件列表。上传及校验成功后以后端 ID 为准并加入 ComposerAttachment；失败 job 直接丢弃，通过现有 composer 错误区域说明原因。缩略图和弹窗原图直接使用项目附件 content endpoint，无需创建 object URL。
