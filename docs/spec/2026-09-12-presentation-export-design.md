...
- 终止交互修订：2026-09-22，支持主动终止、Esc 及右上角关闭退出并清理临时文件，使用“文件已准备好，点击下载到本地。”提示。
...
- 生成过程使用模态层，展示格式、进度条、阶段、页面进度和“终止”操作。
- 准备和生成阶段提供“终止”；ready 阶段只保留“下载”，通过 `Esc` 或右上角关闭放弃产物并清理，不要求用户下载后才能退出；点击遮罩不关闭。
- 终止中显示“正在终止”，禁用重复操作与下载，后端确认取消后关闭模态层；请求失败保留任务和可重试提示，不静默丢弃任务 ID。
- 导出不注册浏览器离开确认，不显示“更改可能未保存”提示；刷新、关闭标签页或离开页面时尽力取消导出并清理临时内容。
- 生成完成后不自动下载，模态层显示“下载”按钮。
- 用户点击下载后，保存位置完全交给浏览器：浏览器可以使用 Downloads，也可以按自身设置询问保存位置。
- 文件成功传输后关闭模态层，删除后端临时文件和一次性任务。
- 下载传输中也可退出弹窗，但不打断浏览器接收文件；成功后清理，若传输失败则取消本次已退出的任务并清理。浏览器是否最终保存到磁盘不属于后端可观测状态。
...
- 不注册 `beforeunload`；`pagehide` 使用 DELETE keepalive 尽力取消，未送达时由心跳超时清理兜底。
- 当前导出弹窗每 15 秒发送一次心跳并同步状态；后端在最后一次心跳超过 90 秒后取消遗留任务，定期检查间隔为 5 秒，创建新导出时也检查过期占用。读取状态和订阅 SSE 本身不续期。
- 页面短暂断线允许重连；超过宽限时间后必须重新导出，不恢复旧任务或旧文件。正在 HTTP 传输的任务不因页面心跳中断而取消；30 分钟总 TTL 仍负责最终清理，包括失败任务的内存记录。
- 取消时等待生成任务停止后再释放项目占用，避免新旧写入并行。
- 后端进程重启时不恢复任务，启动清理遗留导出目录。
- 同项目第二次创建在前一任务未清理前返回 `EXPORT_ALREADY_ACTIVE`。
- 活跃冲突提示原页面完成下载或等待失联任务清理，不伪装成渲染失败，不取消其他仍存活页面的导出。用户重试时创建全新快照。
- 无警告时 REST 和 SSE 的 `warnings` 均返回 `[]`，前端入口归一化空值。
...
该接口由用户点击“终止”、Esc、右上角关闭、`pagehide` 的 keepalive 请求和项目删除流程调用。ready 状态退出时删除尚未下载的文件；已交给浏览器下载时关闭弹窗不立即 DELETE，等待传输成功后的消费清理，或传输失败后再 DELETE。

创建请求尚未返回 export ID 时，前端记录终止意图并保持等待，取得 ID 后立即 DELETE，不只关闭弹窗或中断创建响应。终止期间忽略迟到的进度事件，成功后关闭 SSE 并清空当前会话；失败保留任务，允许再次终止。后续重新导出必须使用新的 client request ID。

### 9.6 页面心跳

~~~http
POST /api/v1/exports/:id/heartbeat
~~~

仅持有当前 export ID 的页面续期，成功返回 `204`；终止或已清理返回 `410`，不能复活已过期任务。前端随后同步 GET 状态，非下载过程收到 `410` 时解除页面锁定并允许重新导出；下载过程中收到 `410` 时关闭弹窗。异步回调按 export ID 匹配，旧任务事件不得覆盖新导出。
...
- 右上角提供公共 IconButton 关闭按钮，并支持 Escape 退出；运行中两者均调用与“终止”相同的取消流程；
- 点击遮罩不关闭；终止请求进行中避免重复提交；
- 显示格式名称、进度条、`已处理 x / y 页` 和当前阶段；
- warning 可显示但不阻塞 HTML；
- 不展示日志、技术堆栈或内部绝对路径。
- 终止期间标题显示“正在终止导出”，按钮显示“正在终止…”且禁用；终止失败提示“终止失败，请检查连接后重试。”并保留操作入口。
...
- 主按钮为“下载”；
- 文案为“文件已准备好，点击下载到本地。”；
- 不显示“终止”按钮；退出时显示“正在清理文件”，而非“正在终止导出”；
- Esc 和右上角关闭同样放弃产物并清理，不强制下载；
- 不自动触发下载；
- 下载传输中按钮禁用并显示“正在交给浏览器…”；
- 下载中关闭仅隐藏弹窗，保留状态订阅；成功后释放会话，失败后清理已放弃任务，清理请求失败时重新显示弹窗供重试；
- 传输失败时恢复“再次下载”。
...
- 导出各状态均不注册 `beforeunload`，避免刷新、离开或下载时出现误导性的未保存确认。
- 页面离开时通过 `pagehide` 尽力发送 DELETE keepalive，保留心跳超时与 TTL 清理作为兜底。
- 下载中主动关闭弹窗后仍沿用传输结束清理流程，不增加离开确认。
- 普通路由项目切换在应用内直接阻止，不弹出第二套自定义确认。
- failed、canceled、consumed 状态移除页面离开监听。
...
- Dialog 支持 Escape 和右上角关闭；准备、生成和 ready 阶段退出即终止清理，后端确认后关闭；遮罩点击不关闭。
- 下载中关闭不提前删除传输中的文件，传输成功或失败后均有清理路径。
- 创建返回前点击终止不会遗漏新建任务，终止失败可以重试，终止期间迟到的 SSE 不复活任务。
- SSE 进度单调更新页数、阶段和进度条。
- 缺页与运行失败展示页面列表且无下载按钮。
- ready 状态不会自动下载，必须点击“下载”。
- delivering 状态按钮禁用；失败后可再次点击。
- 重新导出发送新的 client request ID。
- 导出不触发 beforeunload 确认；pagehide 尽力发送取消，失联任务由后端回收。
...
8. 实现单按钮菜单、支持主动终止的进度 Dialog 和手动浏览器下载。
