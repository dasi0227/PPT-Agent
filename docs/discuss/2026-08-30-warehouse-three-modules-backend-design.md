...
│   │       └── theme.css          # B2：:root token 变量 + 白名单选择器样式
│   └── components/
│       └── <component-name>/
...
|---|---|---|---|---|---|
| **theme** | 强主题性质的全局配色/样式方案（B2 厚契约） | 独立 `manifest.json` | **用户**（UI 换肤） | **只读**（严禁修改主题；可写普通 CSS） | 项目页面 `<link id="theme-link">` 引用仓库真源 |
...
2. **theme 采用 B2 厚契约**：不仅有 `:root` token 变量，还允许对白名单类名（如 `.slide-stage`、`.card`、`.slide-title`）写样式规则，追求 `html-ppt` 式的强主题质感。
...
- theme：以 B2 CSS 文件存储（`:root` token + 白名单选择器样式）+ 独立 `manifest.json` 元数据；后端能列举主题、读取单个主题元数据并从仓库真源提供只读 CSS。
...
theme 是「一系列基本元素的配色与样式方案」，采用 **B2 厚契约**：

- **token 层**：`:root` 下声明设计 token 变量（颜色、字体、间距、圆角、阴影、画布尺寸等），是主题的稳定契约。
- **白名单选择器层**：允许对一组约定的白名单类名（如 `.slide`、`.card`、`.title`、`.subtitle`、`.kicker`、`.list` 等）直接写样式规则，以获得 `html-ppt` 式的强主题质感。
- **降级保障**：即使 Agent 生成的 HTML 类名未命中白名单，只要坚持使用 `var(--token)`，仍能通过 token 层保持整体视觉协调。这是接受 B2 风险的前提。
...
.title { font-size: var(--text-title); color: var(--color-primary); }
```
...
- `base.css` 是 Runtime 内嵌的主题无关结构层，保留 16:9 舞台、基础排版和可访问性降级；不再复制到每个项目的 `common/` 目录。
...
- 必需 token 沿用现有校验基线：`--color-bg`、`--color-fg`、`--color-primary`、`--color-accent`、`--color-muted`、`--font-sans`、`--text-title`、`--text-body`、`--space-4`、`--radius-md`、`--shadow-card`、`--stage-w`、`--stage-h`。
...
- `designsystem/tokens.go` 的 `requiredTokens` / `LintTokens`（token 齐备性）可作为**非阻塞**的辅助校验保留，用于主题健康检查，不用于拦截 Agent 写入。
...
- 列举：返回全部技能（含 name/description/disabled/open_url）。
...
- `backend/seed/assets/themes/**` 改造：由「manifest.json（含 kind/source/assets）+ tokens.css」改为「manifest.json（精简字段）+ theme.css（B2）」，落地根改为 `~/.dasi/ppt/assets/themes/`。
...
| token 齐备校验 | `designsystem/tokens.go`（requiredTokens/LintTokens） | 保留为非阻塞辅助 |
